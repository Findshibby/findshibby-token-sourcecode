...
// File: contracts/FINDSHIBBYDividendPayingToken.sol
...
contract FINDSHIBBY is ERC20, Ownable {
    ...
    FINDSHIBBYDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Findshibby", "FINDSHIBBY") {
        ...
        dividendTracker = new FINDSHIBBYDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "FINDSHIBBY: The dividend tracker already has that address");

        FINDSHIBBYDividendTracker newDividendTracker = FINDSHIBBYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FINDSHIBBY: The new dividend tracker must be owned by the FINDSHIBBY token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FINDSHIBBY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "FINDSHIBBY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "FINDSHIBBY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FINDSHIBBY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FINDSHIBBY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FINDSHIBBY: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            revert("FINDSHIBBY: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
contract FINDSHIBBYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FINDSHIBBY_Dividend_Tracker", "FINDSHIBBY_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "FINDSHIBBY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FINDSHIBBY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FINDSHIBBY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FINDSHIBBY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FINDSHIBBY_Dividend_Tracker: Cannot update claimWait to same value");
